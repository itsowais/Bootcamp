...
*/
function FilteredListItem(props){
  let arr=props.list.filter(val=>val.bookLikedID==props.chk)
  if(arr.length>0){
  return(
    <ul>{
    arr.map(val=>(<li>{props.peeps[val.peepsID].name}</li>))
    }</ul>
  )}
  else{
...
function ListItem(props) {
  return(
<div>
    {Object.keys(props.obj).map(val=>(<div><h2>{props.obj[val].name}</h2><p>Liked by</p><FilteredListItem list={props.chart} chk={props.obj[val].id} peeps={props.peep}/></div>))}
  </div>
  )}
